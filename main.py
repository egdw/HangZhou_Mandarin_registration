...
class Predictor:
    ...
    def preprocess(self):
        ...
        if self.save_path is not "" or None:
    ...
    def parse_code(self):
        try:
            ...
            result = self.reader.readtext_batched(self.image)
            # 合并多个ocr结果
            for r in result:
                ocr_result = ocr_result + r[0][1].strip().lower().replace(" ", "")
...
def circle():
    ...
    predictor = Predictor(save_path=VALIDATE_CODE_SAVE_PATH,  # 验证码保存地址
                          username="XXXXXXXXXXXXXXXXX",  # 用户名
                          passwd="XXXXXXXXXXXX",  # 密码
